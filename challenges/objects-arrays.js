...
for (let i=0; i<graduates.length; i++){
  let allU = graduates[i].university;
  universities.push(allU);
}
...
const contactInfo = [];
for (let i=0; i<graduates.length; i++){
  let allF = graduates[i].first_name;
  let allE= graduates[i].email;
  contactInfo.push(allF + " " + allE);
}
...
forEach(zooAnimals,function(x){
  animalNames.push(x.animal_name + " " + x.scientific_name);
});
...
*/
const lowerCase = [];
const lower = zooAnimals.map((i) => {
  return {"animal_name": i.animal_name.toLowerCase()};
});
...
const total = zooAnimals.reduce((i, add) => {
  return i += add.population}, 0);
